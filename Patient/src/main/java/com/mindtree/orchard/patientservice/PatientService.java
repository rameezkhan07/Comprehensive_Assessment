...
public class PatientService implements PatientInterface {
	...
	public OutputPatient getVisitDetails(Integer id) {
		...
		for(int i=0;i<pat.size();i++) {
			Doctor
			 d=resttemplate.getForObject("http://doctor/doctor/"+pat.get(i).getDocvisited(),Doctor.class);
			sts.add(new Visdp(d.getDocName(),pat.get(i).getDocvisited(),pat.get(i).getPrescription(),pat.get(i).getDate()));
		}
	...
	@Override
	public List<Patient> getDocVisited(Integer id) {
		// TODO Auto-generated method stub
